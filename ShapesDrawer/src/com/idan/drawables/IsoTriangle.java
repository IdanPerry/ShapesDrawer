...
public class IsoTriangle extends Polygon {
     ...
     */
    public IsoTriangle(int originX, int originY, int destX, int destY, int thickness, Color color, boolean filled) {
        super(originX, originY, destX, destY, thickness, color, filled);

        // initiate properties
        setOriginX(originX + (destX - originX) / 2);
        setOriginY(originY);
        setDestX(destX);
        setDestY(destY);
        ...
        setXPoints(new int[]{getOriginX(), originX, destX});
    ...
    public void move(Point origin, Point dest, int currentX, int currentY) {
        ...
        // set new position
        setXPoints(new int[]{ getOriginX(), getOriginX() - getWidth() / 2, getDestX() -  getWidth() / 2});
